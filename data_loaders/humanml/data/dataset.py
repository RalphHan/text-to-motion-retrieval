...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer, labels_file, randomize):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length = 20
        self.pointer = 0
        ...
        # load the file containing the gt labels, if they exist
        # only used if we would like to compute classification metrics
        if os.path.exists(labels_file):
            gt_lab_df = pd.read_csv(labels_file)
            gt_lab_df = gt_lab_df.drop(columns=["descriptions"])

            # convert labels into indexes of the categories
            categories = gt_lab_df['primary_label'].append(gt_lab_df['secondary_label']).append(gt_lab_df['top_level_label'])
            ...
            # merge the two
            gt_lab_df = gt_lab_df.append(gt_lab_df_inv)
        ...
        for name in tqdm(id_list):
            try:
                ...
                text_data = []
                flag = False
                labels_row = gt_lab_df.loc[[name]].to_dict('index')[name] if gt_lab_df is not None else {'null': 0}
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
    ...
    def __getitem__(self, item):
        ...
        motion, m_length, text_data, motion_name, labels = data['motion'], data['length'], data['text'], data['name'], data['labels']
        # Randomly select a caption
        # text_data = random.choice(text_list)
        caption, tokens = text_data['caption'], text_data['tokens']
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        if mode == 'text_only':
            ...
            labels_file = os.path.join(opt.data_root, "labels.csv")
            self.t2m_dataset = Text2MotionDatasetV2(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer, labels_file, randomize)
